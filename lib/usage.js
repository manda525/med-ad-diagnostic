...
// 訪問者あたりの無料診断回数（24時間あたり）。
// 2026-09-24 に「生涯6回で打ち止め→有料プランへ」から「1日5回・翌日に復活」へ変更。
// 課金は機械の一次チェックでなく人の監修で行う方針のため、無料枠は営業の入口として開放する。
export const FREE_HARD_LIMIT = 5;
...
const IP_DAY_LIMIT = 25; // 同じ会社のIPを複数人で共有する想定（5回×5人）

const VISITOR_COOKIE = "mad_vid";
const VISITOR_TTL_SEC = 60 * 60 * 24 * 365;
// 訪問者あたりの利用回数の保持期間。最初の利用から24時間で枠が復活する。
const USAGE_TTL_SEC = 60 * 60 * 24;
...
export async function reserveQuota({ visitorId, ip, pro }) {
  ...
  if (!visitor.ok) {
    return {
      allowed: false,
      status: 402,
      error: "本日の無料診断（5回）を使い切りました。24時間後にまた使えます。急ぎの案件は薬剤師の監修（1万円〜）で対応します。",
